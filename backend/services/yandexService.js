  ...
  try {
    const response = await fleetClient.post("/v1/parks/driver-profiles/list", {
      fields: {
        ...
        driver_profile: ["id", "first_name", "last_name", "phones", "created_date", "work_status"],
        car: ["brand", "model", "number", "year"],
      },
      query: {
        park: {
          id: process.env.YANDEX_PARK_ID,
          driver_profile: { phones: [phone] },
        },
      },
      limit: 1,
      offset: 0,
    ...
    console.error("Yandex getDriverByPhone error:", err.message);
    console.log("Using dummy driver data as fallback");
    return { ...DUMMY_DRIVER, phones: [phone], name: "Driver " + phone.slice(-4) };
  }
};
  ...
  try {
    const response = await fleetClient.post("/v1/parks/driver-profiles/list", {
      ...
      query: {
        park: {
          id: process.env.YANDEX_PARK_ID,
          driver_profile: { id: [yandexDriverId] },
        },
      },
      limit: 1,
      offset: 0,
    ...
    console.error("Yandex getDriverProfile error:", err.message);
    return { ...DUMMY_DRIVER, yandex_id: yandexDriverId };
  }
};
...
const getDriverBalance = async (yandexDriverId) => {
  try {
    ...
    if (!profile) return { balance: 0, currency: "GEL" };
    return { balance: profile.balance, currency: profile.currency };
  } catch (err) {
    console.error("Yandex getDriverBalance error:", err.message);
...
const getDriverTransactions = async (yandexDriverId, daysBack = 30) => {
  try {
    ...
    while (hasMore) {
      const body = {
        query: {
          park: {
            id: process.env.YANDEX_PARK_ID,
            driver_profile: { id: yandexDriverId },
            transaction: { event_at: { from: from.toISOString(), to: now.toISOString() } },
          },
        },
        limit: 100,
      };
      ...
      const response = await fleetClient.post("/v2/parks/driver-profiles/transactions/list", body);
    ...
    console.error("Yandex getDriverTransactions error:", err.message);
    return [];
  }
};
...
const getAllDrivers = async () => {
  try {
    const response = await fleetClient.post("/v1/parks/driver-profiles/list", {
      ...
      limit: 1000,
      offset: 0,
      sort_order: [{ direction: "desc", field: "driver_profile.created_date" }],
    });
    ...
    console.error("Yandex getAllDrivers error:", err.message);
    return [];
  }
};
...
const syncDriverEarnings = async (pool, driverId, yandexDriverId) => {
  try {
    ...
    try {
      ...
      console.error("Failed to update balance from Yandex:", balErr.message);
    }

    return { synced: syncedCount, total_amount: syncedAmount.toFixed(2) };
  } catch (err) {
    console.error("Yandex syncDriverEarnings error:", err.message);
    return { synced: 0, total_amount: "0.00" };
  }
};
